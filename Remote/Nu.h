...
/*!
 @class NuBridgedBlockTestHelper
 @abstract Classmethods for testing cBlock support
 @discussion We cannot test cBlock functionality in Nu itself, so we need an objc helper
 doing the c side testing for us. NuBlocks are passed, called and then results checked
 */
@interface NuBridgedBlockTestHelper : NSObject
/*! Returns the int from the  cBlock generated from the passed NuBlock when passed the param
 */
+(int)intThroughCBlockFromBlock:(NuBlock*)block parameter:(int)param;
@end
...
#pragma mark -
#pragma mark ksjogos nu extensions
#pragma mark Nut Parser

@interface NuNUT : NSObject
-(NSString*)preparseString:(NSString*)string;
@end

#pragma mark Nu Remote
@interface NuRemoteHost : NSObject
+(NuRemoteHost*)hostWithName:(NSString*)name;
-(NuRemoteHost*)initWithName:(NSString*)name;
-(id)parseEval:(NSString*)script;
-(id)eval:(id)code;
@end

@interface NuRemoteClient : NSObject
+(NuRemoteClient*)clientWithName:(NSString*)name;
-(NuRemoteClient*)initWithName:(NSString*)name;
-(id)parseEval:(NSString*)script;
-(id)eval:(id)code;
@end